...
import { jsx, Text, Heading, Container, Paragraph, Card, Donut, Link as SLink, Spinner, AspectRatio, Grid, Box, Flex } from 'theme-ui'
  ...
  return (
    <StarterLayout>
      ...
      <Container>
        ...
        <Paragraph>Provozovatelem webové aplikace ONLIFE (dále jen „webová aplikace“ nebo „tato webová aplikace“) je Filozofická fakulta Masarykovy univerzity, se sídlem Arna Nováka 1/1, 602 00 Brno, IČO 00216224 (dále jen jako „Provozovatel“), která je v souladu se zák. č. 121/2000 Sb., zákon o právu autorském, o právech souvisejících s právem autorským a o změně některých zákonů (dále jen „Autorský zákon“), oprávněna vykonávat majetková práva autorská k obsahu na těchto Internetových stránkách, pokud není uvedeno jinak.</Paragraph>
        <Paragraph>Uživatelem webové aplikace může být každá fyzická či právnická osoba (dále jen „Uživatel“).</Paragraph>
        <Paragraph>Provozovatel tímto vydává Podmínky užívání této webové aplikace (dále jen „Podmínky). Tato webová aplikace je bezplatně veřejně přístupná a její užívání se řídí těmito Podmínkami užívání, které slouží k vymezení vztahu mezi Provozovatelem a Uživatelem této webové aplikace.</Paragraph>
        <Paragraph>Webová aplikace vznikla jako produkt aplikovaného výzkumu TA ČR Platforma pro transfer znalostí: informační gramotnost pro středoškoláky v otevřeném mash-up virtuálním učebním prostředí, realizovaném na Masarykově univerzitě. Zdrojový kód je dostupný jako open source v souladu s podmínkami projektu.
</Paragraph>
        <Paragraph>Veškerá práva k této webové aplikaci náleží Provozovateli, tj. správa kvízů, rozvoj aplikace, práce s daty uživatelů.</Paragraph>
        <Paragraph>Webová aplikace pro ukládání dat uživatelů využívá služby (servery) třetích stran. Autetizaci tj. ověřování identity uživatelů a jejich správu, zajišťuje služba Auth0 (https://auth0.com). Ostatní data jsou uložena v databázi Fauna (https://fauna.com/). Tato data zahrnují veškeré ostatní informace, jako jsou průchod kurzem, výsledky kvízů a vztahy mezi uživateli.</Paragraph>
